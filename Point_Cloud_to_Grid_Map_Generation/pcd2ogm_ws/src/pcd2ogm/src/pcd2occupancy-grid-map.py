...
if __name__ == "__main__":
    ...
    pcd = o3d.io.read_point_cloud("/home/yorgundemokrat/AutonomCar/SLAM/Point_Cloud_to_Grid_Map_Generation/pcd2ogm_ws/src/pcd2ogm/src/testdrive_carla.pcd") # Add here yours ".pcd" file
    pcd_array = np.asarray(pcd.points)

    grid_value = {"OCCUPIED": 100, "FREE": 0, "UNKNOWN": -1, "BORDER": 100}

    resolution = 1.0  # Burada çözünürlük parametresini belirliyoruz
    ...
    rospy.loginfo("Calculated Minimum and Maximum axis data")
    ...
    x_range = int((max_value_x - min_value_x) / resolution)
    y_range = int((max_value_y - min_value_y) / resolution)

    rospy.loginfo("The Dimensions of the Map: {}, {}".format(x_range, y_range))

    points = np.zeros((x_range, y_range), dtype=np.int8)
    axis = np.where(z_axis > 0.1)

    for a in range(len(pcd_array)):
        x_road_axis = int(x_axis[a] / resolution)
        y_road_axis = int(y_axis[a] / resolution)
        new_x_road_axis = x_road_axis - int(min_value_x / resolution)
        new_y_road_axis = y_road_axis - int(min_value_y / resolution)
        
        if new_x_road_axis < x_range and new_y_road_axis < y_range:
            points[new_x_road_axis, new_y_road_axis] = grid_value["FREE"]

    if len(axis[0]) > 0:
        for i in axis[0]:
            x_road_axis = int(x_axis[i] / resolution)
            y_road_axis = int(y_axis[i] / resolution)
            new_x_road_axis = x_road_axis - int(min_value_x / resolution)
            new_y_road_axis = y_road_axis - int(min_value_y / resolution)
            ...
            if new_x_road_axis < x_range and new_y_road_axis < y_range:
                ...
                for r in range(4):
                    try:
                        ...
                        points[new_x_road_axis, new_y_road_axis + r] = grid_value["BORDER"]
                        points[new_x_road_axis + r, new_y_road_axis + r] = grid_value["BORDER"]
                        points[new_x_road_axis - r, new_y_road_axis - r] = grid_value["BORDER"]
                        points[new_x_road_axis + r, new_y_road_axis - r] = grid_value["BORDER"]
                        points[new_x_road_axis - r, new_y_road_axis + r] = grid_value["BORDER"]
                    except IndexError:
    ...
    msg.info.width = points.shape[0]
    msg.info.height = points.shape[1]
    msg.info.origin.position.x = min_value_x
    msg.info.origin.position.y = min_value_y
    msg.info.origin.position.z = 0
    msg.data = points.flatten().tolist()
    emp = np.ones_like(msg.data, dtype=np.uint8)
    cv.imshow("sended_map", np.array(msg.data, dtype=np.uint8).reshape(points.shape))
    print(points.shape)
    cv.waitKey(0)
