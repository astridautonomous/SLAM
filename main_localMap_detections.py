...
class CombinedLidarOGM(Node):
    def __init__(self):
        ...
        # OGM params (Bu veriler kalibre edilebilir)
        self.map_width  = 8
        self.map_height = 8
        self.resolution = 1.0
    ...
    def pointcloud_callback(self, msg):
        ...
        # Build OGM (always) pz kalibre edilebilir
        ...
        # Region checks (yaya geçidi alnına göre kalibre edilecek)
        ...
        # Parking only in parking zone (durak alanlarına göre kalibre edilecek)
        if zone_id is not None:
            self.process_parking(ogm, zone_id)

    def detect_parking_zone(self, x, y):
        if -1.9703<=x<=5.4736 and -10.609<=y<=6.0129:
            return 3
    ...
    def process_parking(self, ogm, zone_id):
        ...
        if self.zone_entry_counts[zone_id]>=2:
            ...
            if black>=4:   # kutucuk sayısı kalibre edilebilir
